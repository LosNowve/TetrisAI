...
from tetris_ai_expmax import TETRIS_AI
...
class Tetris(QMainWindow):
    ...
    def gameOver(self):
        print("Game Over!")
        print("Your score is:", self.tboard.score)
        exit()
...
class SidePanel(QFrame):
    ...
    def paintEvent(self, event):
        ...
        minX, maxX, minY, maxY = BOARD_DATA.nextShape[0].getBoundingOffsets(0)
        for i in range(len(BOARD_DATA.nextShape)):
            dy = 5 * i * self.gridSize
            dx = (self.width() - (maxX - minX) * self.gridSize) / 2

            val = BOARD_DATA.nextShape[i].shape
            for x, y in BOARD_DATA.nextShape[i].getCoords(0, 0, -minY):
